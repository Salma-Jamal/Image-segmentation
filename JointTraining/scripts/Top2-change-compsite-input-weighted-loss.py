...
wandb.init(project="joint-training",
          name="Top2-change-compsite-input-weighted-loss")
...
def dice_coef_loss(predictions, ground_truths, num_classes=4, dims=(1, 2), smooth=1e-8, alpha=0.0):
    ground_truth_oh = F.one_hot(ground_truths, num_classes=num_classes)
    prediction_norm = F.softmax(predictions, dim=1).permute(0, 2, 3, 1)
    intersection = (prediction_norm * ground_truth_oh).sum(dim=dims)
    summation = prediction_norm.sum(dim=dims) + ground_truth_oh.sum(dim=dims)
    ...
    dice_mean = dice.mean()
    CE = F.cross_entropy(predictions, ground_truths)
    dice_losss = (1.0 - dice_mean)
    return  alpha * dice_losss + (1.0 - alpha) * CE


class DiceWeightScheduler:
    """
    Linearly increases α (Dice weight) from α0 to α1
    over `ramp_epochs` epochs, then keeps it fixed.
    """
    def __init__(self, α0=0.2, α1=0.7, ramp_epochs=30):
        self.α0 = α0
        self.α1 = α1
    ...
    def __call__(self, epoch):
        ...
        return self.α0 + (self.α1 - self.α0) * (epoch / self.ramp_epochs)
...
def train_one_epoch(model, dataloader, optimizer, device, scaler,alpha):
    model.train()
    running_loss = 0.0
    ...
    for batch in dataloader:
        ...
        with torch.amp.autocast(device_type='cuda', dtype=torch.float16):
            ...
            loss = dice_coef_loss(pred_masks, query_mask, num_classes=model.num_classes,alpha=alpha)        # your loss stays unchanged
...
def validate(model, dataloader, device,alpha):
    model.eval()
    running_loss = 0.0
    with torch.no_grad():
        for batch in dataloader:
            ...
            with torch.amp.autocast(device_type='cuda', dtype=torch.float16):
                ...
                loss = dice_coef_loss(pred_masks, query_mask, num_classes=model.num_classes,alpha=alpha)
            running_loss += loss.item() * composite_input.size(0)
...
checkpoint_folder = "/home/saahmed/scratch/projects/Image-segmentation/JointTraining/Top2-change-compsite-input-weighted-loss"

scheduler = DiceWeightScheduler(α0=0.2, α1=0.7, ramp_epochs=30)

num_epochs = 50
for epoch in tqdm(range(num_epochs)):
    alpha = scheduler(epoch)
    train_loss = train_one_epoch(joint_module, train_loader, optimizer, device, scaler,alpha)
    valid_loss = validate(joint_module, valid_loader, device,alpha)

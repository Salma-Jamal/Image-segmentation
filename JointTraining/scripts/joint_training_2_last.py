...
wandb.init(project="joint-training",
          name="joint-training_experiment-updated-Apr14-last_experiment",)
...
class JointTrainingModule(nn.Module):
    ...
    def forward(self, query_image):
        ...
        # start_time = time.time()
        # -- 4) "Soft top-k" weighting (in this example, we actually do a soft weighting over all)
        #     Larger similarity => larger weight
        ...
        # guide_image: (B, 3, H, W)
        # guide_image = torch.einsum("bn, bnchw -> bchw", weights, self.cached_gallery_images)
        guide_image = torch.einsum("bn,nchw->bchw", weights, self.cached_gallery_images)
        # guide_mask: (B, H, W)
        # guide_mask = torch.einsum("bn, bnhw -> bhw", weights, self.cached_gallery_masks.float())
...
checkpoint_folder = "/home/saahmed/scratch/projects/Image-segmentation/JointTraining/checkpoints_last"
